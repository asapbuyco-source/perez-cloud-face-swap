...
# Headless server: models live at the repo root next to dlc/
_repo_models = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(abs_dir)))), "models")
if os.path.isdir(_repo_models):
    models_dir = _repo_models
...
def _find_enhancer_model() -> str:
    """DLC-VFX ships gfpgan-1024.onnx; fall back to gfpgan_1.4.onnx (512)
    for servers provisioned with the smaller model. enhance_face() reads the
    actual input size from the session, so either works."""
    for name in ("gfpgan-1024.onnx", "gfpgan_1.4.onnx"):
        p = os.path.join(models_dir, name)
        if os.path.exists(p):
            return p
    return os.path.join(models_dir, "gfpgan-1024.onnx")


def pre_check() -> bool:
    model_path = _find_enhancer_model()
